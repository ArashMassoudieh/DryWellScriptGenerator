...
void MainWindow::On_Generate_Model()
{
    ...
    file.write("setvalue; object=system, quantity=simulation_start_time, value=43927\n");
    ...
    file.write("create source;type=Precipitation,timeseries=/media/arash/E/Dropbox/Drywell Project/Combined_Model/All_rain_data.txt,name=Ave_04082020\n");
...
// Create deep soil layers
    ...
    {   for (unsigned int r = 0; r<GP.nr; r++)
            ...
            {   double x = x_base + 200 + r*300;
                ...
                double r_out = (r+1)*dr + GP.well_radious;
                double area = 3.1415*(pow(r_out,2)-pow(r_in,2));
                file.write(QString("create block;type=Soil,theta_sat=0.4,theta_res=0.05,specific_storage=0.01,x="+QString::number(x)+",Evapotranspiration=,n=1.41,y="+QString::number(y)+",area="+QString::number(area)+",theta=0.2,K_sat="+QString::number(LayerData[layer][1].toDouble()*GP.Ks_factor)+",_width=200,alpha=1,name=Soil_deep ("+QString::number(layer+1)+"$"+QString::number(r+1)+"),_height=100,bottom_elevation="+QString::number(-dy_deep*(layer+1)-GP.well_depth)+",depth="+QString::number(dy_deep)+",actual_x="+QString::number(0.5*(r_in+r_out))+",actual_y="+QString::number(-dy_deep*(layer+0.5)-GP.well_depth)+"\n").toUtf8());
            }
        ...
        for (unsigned int layer=0; layer<GP.n_layer_deep; layer++)
        {
            ...
            double area = 3.1415*(pow(r_out,2)-pow(r_in,2));
            file.write(QString("create block;type=Soil,theta_sat=0.4,theta_res=0.05,specific_storage=0.01,x="+QString::number(0)+",Evapotranspiration=,n=1.41,y="+QString::number(y)+",area="+QString::number(area)+",theta=0.2,K_sat="+QString::number(LayerData[layer][1].toDouble()*GP.Ks_factor)+",_width=100,alpha=1,name=Soil_deep ("+QString::number(layer+1)+"$"+QString::number(0)+"),_height=100,bottom_elevation="+QString::number(-dy_deep*(layer+1)-GP.well_depth)+",depth="+QString::number(dy_deep)+",actual_x="+QString::number(0.5*(r_in+r_out))+",actual_y="+QString::number(-dy_deep*(layer+0.5)-GP.well_depth)+"\n").toUtf8());
        }
    }
    else
    {
        ...
        {   double bottom_elevation = lowest_shallow_depth;
            ...
            {   double x = x_base + 200 + r*300;
                ...
                file.write(QString("create block;type=Soil,theta_sat="+LayerData[layer][4]+",theta_res="+LayerData[layer][5]+",specific_storage=0.01,x="+QString::number(x)+",Evapotranspiration=,n="+LayerData[layer][3]+",y="+QString::number(y)+",area="+QString::number(area)+",theta=0.2,K_sat="+QString::number(LayerData[layer][1].toDouble()*GP.Ks_factor)+",_width=200,alpha="+LayerData[layer][2]+",name=Soil_deep ("+QString::number(layer)+"$"+QString::number(r+1)+"),_height=100,bottom_elevation="+QString::number(bottom_elevation)+",depth="+QString::number(depth)+",actual_x="+QString::number(0.5*(r_in+r_out))+",actual_y="+QString::number(bottom_elevation+depth/2)+"\n").toUtf8());
            }
        }
        double bottom_elevation = lowest_shallow_depth;
        file.write(QString("// ***** Soil Blocks underneath the well ***** //\n").toUtf8());
        for (unsigned int layer=lowest_shallow_layer; layer<LayerData.count(); layer++)
        {
            ...
            double r_out = GP.well_radious;
            double area = 3.1415*(pow(r_out,2)-pow(r_in,2));
            file.write(QString("create block;type=Soil,theta_sat="+LayerData[layer][4]+",theta_res="+LayerData[layer][5]+",specific_storage=0.01,x="+QString::number(0)+",Evapotranspiration=,n="+LayerData[layer][3]+",y="+QString::number(y)+",area="+QString::number(area)+",theta=0.2,K_sat="+QString::number(LayerData[layer][1].toDouble()*GP.Ks_factor)+",_width=100,alpha=1,name=Soil_deep ("+QString::number(layer)+"$"+QString::number(0)+"),_height=100,bottom_elevation="+QString::number(bottom_elevation)+",depth="+QString::number(depth)+",actual_x="+QString::number(0.5*(r_in+r_out))+",actual_y="+QString::number(bottom_elevation+depth/2)+"\n").toUtf8());
        }
...
// Horizontal soil conncetors
    ...
    if (uniform)
    {
        for (unsigned int r = 0; r<GP.nr-1; r++)
            for (unsigned int layer=0; layer<GP.n_layers; layer++)
            {   double radius = (r+1)*dr + GP.well_radious;
    ...
    else
    {
        for (unsigned int r = 0; r<GP.nr-1; r++)
            ...
            {
                double radius = (r+1)*dr + GP.well_radious;
                    file.write(QString("create link;from=Soil ("+QString::number(layer)+"$"+QString::number(r+1)+"),to=Soil ("+QString::number(layer)+"$"+QString::number(r+2)+"),type=soil_to_soil_H_link,name=Soil ("+QString::number(layer)+"$"+QString::number(r+1)+") - Soil ("+QString::number(layer)+"$"+QString::number(r+2)+"),length="+QString::number(dr)+",area="+QString::number(3.1415*radius*LayerData[layer][0].toDouble())+"\n").toUtf8());
            }
    }
...
// Horizontal Deep soil connectors
    ...
    for (unsigned int r = 0; r<GP.nr-1; r++)
    {
        if (uniform)
            for (unsigned int layer=0; layer<GP.n_layer_deep; layer++)
            {   double radius = (r+1)*dr + GP.well_radious;
                file.write(QString("create link;from=Soil_deep ("+QString::number(layer+1)+"$"+QString::number(r+1)+"),to=Soil_deep ("+QString::number(layer+1)+"$"+QString::number(r+2)+"),type=soil_to_soil_H_link,name=Soil_deep ("+QString::number(layer+1)+"$"+QString::number(r+1)+") - Soil_deep ("+QString::number(layer+1)+"$"+QString::number(r+2)+"),length="+QString::number(dr)+",area="+QString::number(3.1415*radius*dy_deep)+"\n").toUtf8());
            }
        else
            for (unsigned int layer=lowest_shallow_layer; layer<LayerData.count(); layer++)
            {   double radius = (r+1)*dr + GP.well_radious;
                ...
                file.write(QString("create link;from=Soil_deep ("+QString::number(layer)+"$"+QString::number(r+1)+"),to=Soil_deep ("+QString::number(layer)+"$"+QString::number(r+2)+"),type=soil_to_soil_H_link,name=Soil_deep ("+QString::number(layer)+"$"+QString::number(r+1)+") - Soil_deep ("+QString::number(layer)+"$"+QString::number(r+2)+"),length="+QString::number(dr)+",area="+QString::number(3.1415*radius*depth)+"\n").toUtf8());
            }
...
void MainWindow::On_File_Select()
{
...
void MainWindow::On_ReadLayer_Info()
{
...
void MainWindow::On_CreateCADDrawing()
{
    double gap = 0.05;
    GP.depth_to_gw = ui->Depth_to_GW->text().toDouble();
    GP.n_layer_deep = ui->nl_deep->text().toInt();
