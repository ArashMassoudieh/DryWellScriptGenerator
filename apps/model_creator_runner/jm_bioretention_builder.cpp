...
QString BuildReference(const StarterScriptOptions &options)
{
    ...
    ts << "# JM blocks: media/sump soil=" << (nx * 2) << ", aggregate_storage=" << (nx * 2)
    ...
    // The longitudinal gravel cells are already connected in sequence. Connect
    // only the terminal gravel cell to the single underdrain block so the
    // topology remains strictly adjacent and does not form a star.
    ts << "create link;from=JM Gravel (" << nx
       << "),to=JM Underdrain,type=aggregate2aggregate_H_Link,length=" << n(dx)
       << ",name=JM Gravel - Underdrain,width=" << n(width) << "\n";

    // The bottom native-soil cells are also connected longitudinally. Use one
    // terminal groundwater boundary connection instead of linking every
    // surrounding soil block directly to the same fixed-head block.
    ts << "create link;from=JM Bottom Native Soil (" << nx
       << "),to=JM Groundwater,type=soil_to_fixedhead_link,name=JM Bottom Native Soil - Groundwater\n";
